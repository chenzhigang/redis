...
public class NearbyService {
    ...
    public void addPerson(PersonModel personModel) {
        stringRedisTemplate.opsForGeo().add(PERSON_KEY_STR, new Point(personModel.getLongitude().doubleValue(), personModel.getLatitude().doubleValue()), JSON.toJSONString(personModel));
    }
    ...
    public List<PersonModel> getNearByPersonList(NearbyModel nearbyModel) {
        ...
        List<GeoResult<RedisGeoCommands.GeoLocation<String>>> content = Objects.requireNonNull(stringRedisTemplate.opsForGeo().radius(PERSON_KEY_STR, circle)).getContent();
        return content.stream().map(it -> JSON.parseObject(it.getContent().getName(), PersonModel.class)).toList();
    }
}
